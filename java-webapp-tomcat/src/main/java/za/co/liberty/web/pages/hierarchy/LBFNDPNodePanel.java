...
/**
 * LBF NDP Hierarchy node details panel, capturing all hierarchy node details
 * 
 * @author MZL2611
 * 
 */
public class LBFNDPNodePanel extends BasePanel implements ISecurityPanel{	
	
	private static final Logger logger = Logger.getLogger(LBFNDPNodePanel.class);
...
//	TODO add all form components to this list to validate before controller validates
	 ...
	 * @param arg0
	 */
	public LBFNDPNodePanel(String id, final MaintainHierarchyPageModel pageModel,
		...
		HierarchyNodeDTO parentNode = (HierarchyNodeDTO) SerializationUtils.clone(pageModel.getPartyDTOBeforeImage());
		
		//LBF NDB Change additional branch of 
		List<HierarchyLBFNDPLinkDTO> lbfNodes = new ArrayList<HierarchyLBFNDPLinkDTO>();
		HierarchyLBFNDPLinkDTO lbfNode = new HierarchyLBFNDPLinkDTO();
		lbfNode.setEffectiveFrom(Calendar.getInstance().getTime());
		lbfNode.setSelectedParty(parentNode);
		lbfNode.setHierarchyType(pageModel.getAddBranchTypeList().get(0));
		lbfNodes.add(lbfNode);
		pageModel.getLbfNDPPartyDTO().setLinkedLBFNDP(lbfNodes);
		
//		set the cost center in the page model
		if(pageModel != null && pageModel.getLbfNDPPartyDTO() != null && pageModel.getLbfNDPPartyDTO().getCostCenter() != null){
			pageModel.setCostCenterSelection(pageModel.getLbfNDPPartyDTO().getCostCenter().getName());
		}else{
	...
	private void initialiseModel() {
		pageModel.getLbfNDPPartyDTO().setBranchCode(null);
		//pageModel.getLbfNDPPartyDTO().setStatus(null);
		pageModel.getLbfNDPPartyDTO().setEffectiveFrom(null);
		pageModel.getLbfNDPPartyDTO().setEffectiveTo(null);
		pageModel.getLbfNDPPartyDTO().setOid(0);
		pageModel.getLbfNDPPartyDTO().setRegDateFrom(null);
		pageModel.getLbfNDPPartyDTO().setRegistrationNumber(null);
		pageModel.getLbfNDPPartyDTO().setTerminationDate(null);
		pageModel.getLbfNDPPartyDTO().setVersion(0);
		List<HierarchyNodeLinkDTO> parents = pageModel.getLbfNDPPartyDTO().getParents();
		for (HierarchyNodeLinkDTO hierarchyNodeLinkDTO : parents) {
			hierarchyNodeLinkDTO.setEffectiveFrom(Calendar.getInstance().getTime());
		}
		List<HierarchyEmployeeLinkDTO> linkedEmployees = pageModel.getLbfNDPPartyDTO().getLinkedEmployees();
		for (HierarchyEmployeeLinkDTO hierarchyEmployeeLinkDTO : linkedEmployees) {
			hierarchyEmployeeLinkDTO.setEffectiveFrom(Calendar.getInstance().getTime());
		}
	...
	public class NodeForm extends Form {
		...
		public NodeForm(String id) {
			...
			this.add(nodes);
			this.add(nodeGrid = createParentNodeGrid("nodeList", pageModel.getLbfNDPPartyDTO()));
			this.add(lbfndpGrid = createLBFNDPGrid("lbfndpList", pageModel.getLbfNDPPartyDTO()));
			this.add(employeeGrid = createEmployeeGrid("employeeList", pageModel.getLbfNDPPartyDTO()));
				...
				public void validate(final Form form) {				
					...
					if(validate){
						...
						try{	
							...
							validateLBFNDB(pageModel.getLbfNDPPartyDTO());
							controller.validateNodeWithoutContactDetail(pageModel.getLbfNDPPartyDTO());
		...
		protected void validateLBFNDB(HierarchyNodeDTO lbfNDPPartyDTO) throws ValidationException {
			List errors = new ArrayList<String>();
			if(lbfNDPPartyDTO.getExternalType().getKeyInt() == 2){
				if(!lbfNDPPartyDTO.getBusinessName().contains("LBF")){
					errors.add("The name must contain LBF");
					throw new ValidationException(errors);
				}
			}
			if(lbfNDPPartyDTO.getExternalType().getKeyInt() == 16){
				if(!lbfNDPPartyDTO.getBusinessName().contains("NDP")){
					errors.add("The name must contain NDP");
					throw new ValidationException(errors);
				}
			}
		}		
	...
	private WebMarkupContainer createRolesView(String id){
		...
		if(pageModel == null || pageModel.getLbfNDPPartyDTO() == null 
				|| !(pageModel.getLbfNDPPartyDTO() instanceof HierarchyNodeDTO) 
				|| pageModel.getLbfNDPPartyDTO().getId() == 0
	...
	private HelperPanel createTerminationDateField(){				
		final HelperPanel endDate = createPageField(pageModel.getLbfNDPPartyDTO(), "Termination Date", "terminationDate", ComponentType.DATE_SELECTION_TEXTFIELD, true,true,new EditStateType[]{EditStateType.TERMINATE});
		return endDate;
	}
	...
	private Label createStatusField(){
		Label status = new Label("status",(pageModel.getLbfNDPPartyDTO().getStatus() != null) ? pageModel.getLbfNDPPartyDTO().getStatus().getName() : "Unknown");
		return status;
	}
	...
	private Label createStatusChangeDateField(){
		...
		Label status = new Label("statusChangeDate",(pageModel.getLbfNDPPartyDTO().getCreationTime() != null) ? dteFormatter.format(pageModel.getLbfNDPPartyDTO().getCreationTime()) : "");
		return status;
	}
	...
	private Label createEndDateField(){
		Label status = new Label("effectiveTo",(pageModel.getLbfNDPPartyDTO().getEffectiveTo() != null) ? dteFormatter.format(pageModel.getLbfNDPPartyDTO().getEffectiveTo()) : "");
		return status;
	}
...
//		window.setPageMapName("EditAddressPageMap");
	...
	private HelperPanel createStartDateField(){
		HelperPanel startDate = createPageField(pageModel.getLbfNDPPartyDTO(), "Start Date","effectiveFrom", ComponentType.DATE_SELECTION_TEXTFIELD, true,true,new EditStateType[]{EditStateType.ADD});
...
//		add in popup op current branch names so user does not enter same/similar name twice			
		AutoCompleteTextField nameField = new AutoCompleteTextField(
				"value", new PropertyModel(pageModel.getLbfNDPPartyDTO(),
				"businessName"),
			...
			protected Iterator getChoices(String input) {
				...
				try {
					if(pageModel.getLbfNDPPartyDTO() != null && pageModel.getLbfNDPPartyDTO().getType() != null){
						parties = pageModel.getHierarchyGUIController().findPartyWithOrganisationNameOfType(input + "*",pageModel.getLbfNDPPartyDTO().getType().getOid());
					}else{
			...
			public void doProcessSelectedItems(AjaxRequestTarget target,
				...
				for (ResultContextItemDTO contextItemDTO : selectedItemList) {
					...
					if(resultPartyDTO.getTypeOid() != SRSType.PERSON){
						LBFNDPNodePanel.this.error("Employee's must be people");		
...
//		win.setPageMapName("employeeSearchPageMap");
	...
	private HelperPanel createBranchCodeField() {
		HelperPanel ret = createPageField(pageModel.getLbfNDPPartyDTO(),
	...
	private void changeChannelNameField(WebMarkupContainer channelName){
		if(pageModel != null && pageModel.getLbfNDPPartyDTO() != null && pageModel.getLbfNDPPartyDTO().getType() != null && pageModel.getLbfNDPPartyDTO().getType().getOid() != SRSType.DIVISION){
	...
	private HelperPanel createTypeField() {
		SRSDropDownChoice dropDownChoice = new SRSDropDownChoice("value",
				new PropertyModel(pageModel.getLbfNDPPartyDTO(), "type"),
	...
	private HelperPanel createExternalType() {
		SRSDropDownChoice dropDownChoice = new SRSDropDownChoice("value",
				new PropertyModel(pageModel.getLbfNDPPartyDTO(), "externalType"),
		...
		dropDownChoice.setRequired(true);
		externalType = createPageField("externalType","ExternalType", dropDownChoice,new EditStateType[]{EditStateType.VIEW, EditStateType.VIEW});
...
//				remove parent node
				oldParents = null;
				if(pageModel.getLbfNDPPartyDTO().getParents().size() != 0){
					oldParents = new ArrayList<HierarchyNodeLinkDTO>(pageModel.getLbfNDPPartyDTO().getParents());
					pageModel.getLbfNDPPartyDTO().getParents().clear();
				}
			}else{	
				//add parent node
				if(pageModel.getLbfNDPPartyDTO().getParents().size() == 0){
					if(oldParents == null || oldParents.size() == 0){
						...
						pageModel.getLbfNDPPartyDTO().getParents().add(newlink);
					}else{
						pageModel.getLbfNDPPartyDTO().getParents().addAll(oldParents);
						oldParents = null;
					}
				}
			}	
	...
	private void changeCostCenterNameField(WebMarkupContainer costCenterNameComponent){
		if(pageModel != null && pageModel.getLbfNDPPartyDTO() != null && pageModel.getLbfNDPPartyDTO().getType() != null && pageModel.getLbfNDPPartyDTO().getType().getOid() == SRSType.BRANCH){
	...
	private void setUpParentbutton(Button button){
		if(getEditState() == EditStateType.AUTHORISE){
			button.setVisible(false);
				...
				&& pageModel != null && pageModel.getLbfNDPPartyDTO() != null 
				&& pageModel.getLbfNDPPartyDTO().getType() != null 
				&& pageModel.getLbfNDPPartyDTO().getType().getOid() != SRSType.DIVISION){
...
//	}
		...
		Model<IPartyNameAndIdFLO> dropdownModel = new Model<IPartyNameAndIdFLO>(){
			...
			public IPartyNameAndIdFLO getObject() {
				if(pageModel.getLbfNDPPartyDTO() == null || pageModel.getLbfNDPPartyDTO().getChannel() == null){
					return null;
				}
				...
				for(IPartyNameAndIdFLO flo : pageModel.getHierarchyChannelList()){
					if(flo.getOid() == pageModel.getLbfNDPPartyDTO().getChannel().getOid()){
						return flo;
					}
				}
				return null;
			...
			public void setObject(IPartyNameAndIdFLO flo) {
				if(flo == null){
					...
					pageModel.getLbfNDPPartyDTO().setChannel(null);
					return;
				}
				//look up ResultPartyDTO
				try {
					pageModel.getLbfNDPPartyDTO().setChannel(getPartyManagement().findPartyWithObjectOid(flo.getOid()));					
				} catch (DataNotFoundException e) {	
	...
	private void updateParentTypeList(AjaxRequestTarget target) {		
		ResultPartyDTO channel = pageModel.getLbfNDPPartyDTO()
				.getChannel();
		IDValueDTO type = pageModel.getLbfNDPPartyDTO().getType();	
...
//						}
		...
		if(target != null){	
			...
			for(HierarchyNodeLinkDTO parent : pageModel.getLbfNDPPartyDTO().getParents()){
				updateData(parent);				
			}	
			target.add(nodeGrid);	
		}
	...
	private Button createAddFutureParentButton(String id) {
		...
		button.add(new AjaxFormComponentUpdatingBehavior("click") {
			...
			@Override
			protected void onUpdate(AjaxRequestTarget target) {					
				if (pageModel.getLbfNDPPartyDTO().getParents().size() != 2) {					
					HierarchyNodeLinkDTO dto = new HierarchyNodeLinkDTO();
					dto.setEffectiveFrom(new Date());
					List<Long> parentTypes = pageModel.getHierarchyGUIController().getAllowableHierarchyParentTypesForChildTypes(pageModel.getLbfNDPPartyDTO().getType().getOid());
					if(parentTypes != null && parentTypes.size() != 0){
						dto.setType(pageModel.getTypeFromList(parentTypes.get(0)));						
					}					
					pageModel.getLbfNDPPartyDTO().getParents().add(dto);					
					target.add(nodeGrid);
				} else {
					error("Only one future parent is allowed");
					if (LBFNDPNodePanel.this.feedBackPanel != null) {
						target.add(LBFNDPNodePanel.this.feedBackPanel);
					}
				}
			}
		});		
	...
	private Button createRemoveFutureParentButton(String id) {
		...
		button.add(new AjaxFormComponentUpdatingBehavior("click") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				...
				//currentParent = getCurrentParentNode();				
				//HierarchyNodeLinkDTO main = pageModel.getLbfNDPPartyDTO()
				//		.getParents().get(0);
				for (Object selection : selections) {
					...
					if (selection != currentParent) {
						pageModel.getLbfNDPPartyDTO().getParents()
								.remove(selection);
					}
				}
				if (pageModel.getLbfNDPPartyDTO().getParents().size() == 1
						&& selections.size() != 0) {
		...
		button.add(new AjaxFormComponentUpdatingBehavior("click") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				...
				for (Object link : links) {
					pageModel.getLbfNDPPartyDTO().getLinkedEmployees()
							.remove(link);
				}
				target.add(employeeGrid);
			}
	...
	private Button createAddEmployeeButton(String id) {
		...
		button.add(new AjaxFormComponentUpdatingBehavior("click") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				...
				dto.setEffectiveFrom(new Date());
				pageModel.getLbfNDPPartyDTO().getLinkedEmployees().add(
						dto);
				target.add(employeeGrid);
			}
		});
...
//				});
	...
	private SRSDataGrid createEmployeeGrid(String id, HierarchyNodeDTO dto) {
		...
		for (HierarchyEmployeeLinkDTO hierarchyEmployeeLinkDTO : nodes) {
			if(hierarchyEmployeeLinkDTO.getHierarchyType().getOid() == SRSType.ISRUNBY){
				hierarchyEmployeeLinkDTO.getHierarchyType().setOid(SRSType.ISCOMANAGER);
				hierarchyEmployeeLinkDTO.getHierarchyType().setName("CoManager");
...
//							IDValueDTO id = (IDValueDTO) super.getObject();
//							if(id == null){
//								return null;							
//							}
//							for(IDValueDTO type : pageModel.getEmployeeTypeList()){
//								if(type.getOid() == id.getOid()){
//									return type;
//								}
//							}
//							return null;
...
//							super.setObject(((IDValueDTO)arg0));
//						}
//				},pageModel.getEmployeeTypeList(),new ChoiceRenderer("name", "oid"));
//				dropdown.add(new AjaxFormComponentUpdatingBehavior("change"){
				...
				dropdownPanel.getEnclosedObject().add(new AjaxFormComponentUpdatingBehavior("change") {
					...
					protected void onUpdate(AjaxRequestTarget target) {
						//do nothing just want the object value updated
//						if(data.getHierarchyType().getOid() == SRSType.ISSUPERVISOROF && pageModel.getEmployeeTypeList().size() == 1){
//							//selecting first one if list is only 1 size big
//							data.getHierarchyType().setOid(pageModel.getEmployeeTypeList().get(0).getOid());							
//						}else{
//							data.getHierarchyType().setOid(SRSType.ISADMINISTEREDBY);
//						}
...
//		add search button, don't display this column on view
	...
	private HierarchyNodeLinkDTO getCurrentParentNode(){
		...
		List<HierarchyNodeLinkDTO> currentParents = TemporalityHelper.getInstance().getCurrentObjects(new Date(),pageModel.getLbfNDPPartyDTO().getParents());	
		if(currentParents.size() > 0){
			currentParent = currentParents.get(0);
		}
		if(currentParent == null && pageModel.getLbfNDPPartyDTO().getParents().size() > 0){
			currentParent = pageModel.getLbfNDPPartyDTO().getParents().get(0);
		}	
		return currentParent;
	}
		...
		window.setPageCreator(new ModalWindow.PageCreator() {
			...
			public Page createPage() {					
				...
				if(popupType == Popup_Type.CHILDREN){
					...
					return new HierarchyLinkedAgreementsPopupPage(pageModel.getLbfNDPPartyDTO());		
				}
			}
		});		
	...
	private void updateData(HierarchyNodeLinkDTO data){		
		...
		long channel = (pageModel != null && pageModel.getLbfNDPPartyDTO() != null && pageModel.getLbfNDPPartyDTO().getChannel() != null) ? pageModel.getLbfNDPPartyDTO().getChannel().getPartyOid() : 0;
		...
		if(type != SRSType.DIVISION){				
			...
			nodes.add(createNewPartyNameAndIdFLO(pageModel.getLbfNDPPartyDTO().getChannel()));
		}	
